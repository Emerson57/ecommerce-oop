...
/// <summary>
/// Validador de aplicación para el comando <see cref="UpdateCartItemQuantityCommand"/>.
/// </summary>
/// <remarks>
/// Esta clase centraliza las reglas de validación de entrada necesarias
/// antes de ejecutar el caso de uso de actualización de la cantidad
...
public sealed class UpdateCartItemQuantityCommandValidator : AbstractValidator<UpdateCartItemQuantityCommand>
{
    ...
    /// <summary>
    /// Cantidad mínima permitida en una actualización de ítem.
    /// </summary>
    /// <remarks>
    /// El valor cero se permite porque puede representar la eliminación
    /// de la línea del carrito según las reglas del caso de uso.
    /// </remarks>
    private const int MinQuantity = 0;

    /// <summary>
    /// Cantidad máxima permitida en una actualización de ítem.
    /// </summary>
    /// <remarks>
    ...
    /// <see cref="UpdateCartItemQuantityCommandValidator"/>.
    /// </summary>
    ...
    /// del carrito, del ítem, del producto y del solicitante.
    /// </summary>
    private void ConfigureIdentityRules()
    {
        ...
        RuleFor(x => x.CartItemId)
            .NotEmpty()
            .WithMessage("El identificador del ítem del carrito es obligatorio.");
    ...
    /// <summary>
    /// Configura las reglas relacionadas con la operación de actualización.
    /// </summary>
    private void ConfigureOperationRules()
    {
        RuleFor(x => x.NewQuantity)
            .GreaterThanOrEqualTo(MinQuantity)
            .WithMessage($"La nueva cantidad del ítem no puede ser menor que {MinQuantity}.")
            .LessThanOrEqualTo(MaxQuantity)
            .WithMessage($"La nueva cantidad del ítem no puede superar {MaxQuantity} unidades.");
    }
    ...
    private void ConfigureCrossFieldRules()
    {
        ...
        RuleFor(x => x)
            .Must(command => command.CartId != command.CartItemId)
            .WithMessage("El identificador del carrito y el identificador del ítem del carrito no pueden coincidir.");

        RuleFor(x => x)
            .Must(command => command.CartItemId != command.ProductId)
            .WithMessage("El identificador del ítem del carrito y el identificador del producto no pueden coincidir.");
    }
